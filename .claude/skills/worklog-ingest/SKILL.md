...
2. 匹配 `projects.overrides` 某条且该条带 `level` → 该 level（`match` 为 glob 模式，对项目**绝对路径**匹配、`~` 先展开；`**` 匹配任意深度，如 `~/work/**` 命中其下所有项目）
...
- `summary`：一句概要 + 计数，不建项目页；时间线聚合为一行（如「beta-lib：2 commits」），不逐条展开主题
- `presence`：只记「存在，N commits」，**不读 commit 内容**；时间线最多同样一行；窗口内（identity 过滤后）0 commits 且 0 dirty 且无 MTIME_ACTIVE = 无信号，日记可省略不提；有他人活动等有意义信号可提一句
- `exclude`：**全部 git 追踪产物零出现**（日记与 wiki 含 log.md）；记录排除动作只写计数（如「排除 1 项目」），项目名只允许落在 .gitignore 内的本地文件（status / history log）
...
**结构（软契约，允许用户演化；标题按 config `diary_title_template`，其中 `{weekday}` 解析：zh = 周X（与 frontmatter `day` 及日期门一致）、en = 英文全名如 Saturday）**：

```
frontmatter: date / day(周X 或 weekday) / projects[](= 正文提及的项目 slug，含 presence 级列出的) / git_commits(= 日记涉及项目的 commit 总数，不含 exclude 级)
标题行
...
- 主线是「推进了什么」不是「碰了什么」；有决策必写 why 与否决项。**why 只能来自素材原文**：commit / brain-dump 说了什么就转述什么（「for simpler ops」→「为省运维」可以），素材没给理由就如实写「理由素材未述」+ ⚠️，**禁止推测补全**（素材没说「规模不需要分布式」「客户无 PDF 刚需」这类话，一个字都不能替用户编，编造的理由会沉淀进 wiki 决策日志毒化复盘）
- 时间只用可验证的：brain-dump 给模糊时段（「下午」「晚上」）就按原始措辞入时间线，不编造具体钟点；确需估算位置排序时标 ⚠️ 及推断依据（如「⚠️ 据 16:00 commit 反推」），让读者能区分实测时间与推断时间
...
- **星期与日期必须工具换算**（`python3 -c` 或 date 命令，先 export TZ），禁止心算；写完跑 `date_weekday_check.py`（zh / en 写法均校验）
...
3. `wiki/projects/<slug>.md`（仅 detail 级）：不存在则按 CONVENTIONS 的 frontmatter 契约创建；刷 `last_updated`、`source_count` +1、`diaries` 数组追加；有决策的在「决策日志」段追加一条（日期 + 决策 + why；why 同 D.1 纪律：只转述素材，素材未述写「理由素材未述」+ ⚠️，绝不推测补全）
...
| 上次运行崩溃残留（开跑时 git status 非净） | 残留改动先单独 commit（`chore: 恢复上次中断的改动`）再开跑，绝不混进本次 ingest commit |
