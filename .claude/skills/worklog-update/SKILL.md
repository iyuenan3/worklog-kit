...
3. **差异盘点**：白名单 = 上游 `.claude/skills/` 下全部子目录。逐 skill 与本地 `diff -rq`，列出 新增（上游新 skill，供用户选择装不装）/ 有变更（**明确列出「本地存在但上游已删」的文件**，镜像同步会删掉它们）/ 本地独有 skill（用户自己加的，永不动）；摘录上游 `AIREADME/CHANGELOG.md` 相关段给用户看「这次更新带来什么」（kit 发布史与 vault 版本流合在该文件，读上游 clone 里的那份、不动本地）。
