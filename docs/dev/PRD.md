...
sources:
  ...
  - {type: remote-ssh, host: workbox, roots: [~/work], assume: active, on_unreachable: note}   # roots 必填：scan.sh 需要远端扫描根
...
maintenance:                     # vault 体检阈值（§18；全部可选，缺省用脚本内置默认值）
  drift_days: 14                 # 另有 orphan_days: 60 / bloat_kb: 50 / decision_log_max: 30 / todo_stale_days: [30, 90]
...
- **M6 vault 记忆维护**（✅ 2026-07-12 完成，§18 全部四件，含 6 维对抗验证 21 条原始收敛 3 条确认全处置）：lint 健康节（--health 五项 + config maintenance 阈值段 + 14 例 pytest）+ ingest 体检提醒行（三路径：追加 / 同日清零原地改写 / 健康零痕迹）+ worklog-maintain skill（双 locale 路由）+ 文档三小件；fixture vault E2E 实跑验收：五类腐坏注入 6 项全检出 → 5 类修复各一 commit（6 → 1 → 0，「仍有效留给用户」路径如实保留）→ 标点门 / 日期门 / 正误节 / 体检四门全绿
