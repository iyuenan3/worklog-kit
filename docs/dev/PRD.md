...
- **M6 vault 记忆维护**（✅ 2026-07-12 完成，§18 全部四件）：lint 健康节（--health 五项 + config maintenance 阈值段 + 12 例 pytest）+ ingest 体检提醒行（三路径：追加 / 同日清零原地改写 / 健康零痕迹）+ worklog-maintain skill（双 locale 路由）+ 文档三小件；fixture vault E2E 实跑验收：五类腐坏注入 6 项全检出 → 5 类修复各一 commit（6 → 1 → 0，「仍有效留给用户」路径如实保留）→ 标点门 / 日期门 / 正误节 / 体检四门全绿
