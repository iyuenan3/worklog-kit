...
4. （核心 ingest 就绪后）每晚睡前说「记录今天」+ 一句话补充，然后去睡

## 状态

**pre-alpha（2026-07-11 立项）**：M1 模板骨架已就位（初始化可用）；核心 ingest（M2）与连接器（M3）开发中，「记录今天」尚不可用。规格见 [docs/dev/PRD.md](docs/dev/PRD.md)，方法论全文见 [docs/methodology.md](docs/methodology.md)。
